...
function bikeIcon(count) {
  const icon = '<svg class="bike" viewBox="0 0 32 28" aria-hidden="true"><use href="assets/ui-icons.svg?v=67#scooter"></use></svg>';
...
function metricIcon(id) {
  return `<span class="metricIconSlot" aria-hidden="true"><svg class="metricIcon" viewBox="0 0 24 24"><use href="assets/ui-icons.svg?v=67#${id}"></use></svg></span>`;
}
...
function drawCards(values) {
  ...
  $("metrics").innerHTML = ids.map((id, index) => {
    ...
    const switchIcon = toggle ? '<svg class="paceSwitchIcon" viewBox="0 0 24 24" aria-hidden="true"><use href="assets/ui-icons.svg?v=67#swap"></use></svg>' : "";
...
if ("serviceWorker" in navigator) {
  addEventListener("load", () => navigator.serviceWorker.register("sw.js?v=67").catch(() => {}));
}
